...
public class JacksonAggregateRootMaterializedStatesSerializer implements AggregateRootMaterializedStatesSerializer {
    ...
    public String serialize(final AggregateRoot aggregateRoot, final boolean shouldEncrypt) {
        ...
        return jacksonAggregateRootMaterializedStateSerializerBeans.stream()
                ...
                .findFirst()
                .map(jacksonAggregateRootMaterializedStateSerializerBean -> jacksonAggregateRootMaterializedStateSerializerBean.encode(aggregateRoot, shouldEncrypt, this.objectMapper))
                .map(jsonNode -> {
                    try {
